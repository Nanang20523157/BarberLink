...
class PasswordStepViewModel(
    ...
    private var reAuthenticationAccount: Boolean = false
    ...
    fun setReAuthenticationAccount(value: Boolean) {
        viewModelScope.launch {
            reAuthenticationAccount = value
        }
    }
    ...
    fun getReAuthenticationAccount(): Boolean {
        return runBlocking {
    ...
    fun createNewAccount(email: String, password: String) {
        viewModelScope.launch {
            ...
            if (!NetworkMonitor.isOnline.value) {
                ...
                _registerResult.postValue(ResultState.ShowToast("", true)) // Loadingnya belum nyala
//            _registerResult.postValue(ResultState.ShowToast("Koneksi internet tidak tersedia. Periksa koneksi Anda.", false))
                return@launch
            }

            if (loginType == "Login as Admin") _registerResult.value = ResultState.ShowToast("Menginisiasi akun barbershop...", false)
            else _registerResult.value = ResultState.ShowToast("Menginisiasi akun pegawai...", false)

            val errorMessage = if (loginType == "Login as Admin") "Gagal menginisiasi akun barbershop!" else "Gagal menginisiasi akun pegawai!"
            try {
                auth.createUserWithEmailAndPassword(email, password)
                    .addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            ...
                            if (loginType == "Login as Admin") userAdminCopy = userAdminData.copy()
                            else userEmployeeCopy = userEmployeeData.copy()
                            if (user != null) {
                                if (loginType == "Login as Admin") userAdminData.uid = user.uid
                                else userEmployeeData.uid = user.uid
                                addNewAccountUserToDatabase()
                            } else {
                                _registerResult.postValue(ResultState.Failure(errorMessage, "RETRIEVE_UID"))
                            }
                        } else {
                            _registerResult.value = ResultState.ShowToast(errorMessage, true)
                        }
                    }
            } catch (e: Exception) {
                _registerResult.value = ResultState.ShowToast(errorMessage, true)
...
//            _registerResult.postValue(ResultState.Failure("Koneksi internet tidak tersedia. Periksa koneksi Anda.", "RETRIEVE_UID"))
            ...
            try {
                if (reAuthenticationAccount) if (loginType == "Login as Admin") userAdminCopy = userAdminData.copy() else userEmployeeCopy = userEmployeeData.copy()
                val isEmailSame = if (loginType == "Login as Admin") (userAdminCopy.email == userAdminData.email) else (userEmployeeCopy.email == userEmployeeData.email)
                val isPasswordSame = if (loginType == "Login as Admin") (userAdminCopy.password == userAdminData.password) else (userEmployeeCopy.password == userEmployeeData.password)
                if (isEmailSame && isPasswordSame) {
                        ...
                        .addOnCompleteListener { signInTask ->
                            if (signInTask.isSuccessful) {
                                ...
                                if (user != null) {
                                    ...
                                    _registerResult.postValue(ResultState.Failure("Terjadi kesalahan saat menyiapkan data yang dibutuhkan!", "RETRIEVE_UID"))
                                }
                            } else {
                                _registerResult.postValue(ResultState.Failure("Terjadi kesalahan saat menyiapkan data yang dibutuhkan!", "RETRIEVE_UID"))
                            }
                    ...
                    val user = auth.currentUser
                    val oldPassword = if (loginType == "Login as Admin") userAdminCopy.password else userEmployeeCopy.password
...
//        }.execute() // Pastikan untuk mengeksekusi AsyncTask
    ...
    @RequiresApi(Build.VERSION_CODES.S)
    fun addNewAccountUserToDatabase() {
        viewModelScope.launch {
            _registerResult.postValue(ResultState.Loading)
            // PENGECHECKAN MANUAL KARENA GAK OFFLINEAWARE
            if (!NetworkMonitor.isOnline.value) {
...
//            _registerResult.postValue(ResultState.Failure("Koneksi internet tidak tersedia. Periksa koneksi Anda.", "UPLOAD_IMAGE"))
            ...
            if (loginType == "Login as Admin") _registerResult.value = ResultState.ShowToast("Membuat akun barbershop!", false)
            else _registerResult.value = ResultState.ShowToast("Membuat akun pegawai!", false)

            val errorMessage = if (loginType == "Login as Admin") "Terjadi kesalahan saat mengunggah gambar barbershop!" else "Terjadi kesalahan saat mengunggah profil pegawai!"
            imageUri?.let {
//            Toast.makeText(this, "Uploading Image...", Toast.LENGTH_SHORT).show()
                // Upload image to Firebase Storage
                try {
                    val userUID = if (loginType == "Login as Admin") userAdminData.uid else userEmployeeData.uid
                    val storageRef = storage.reference.child("profiles/$userUID")
                    storageRef.putFile(it)
                        .addOnSuccessListener {
                            storageRef.downloadUrl.addOnSuccessListener { url ->
                                ...
                                _registerResult.postValue(ResultState.Failure(errorMessage, "UPLOAD_IMAGE"))
                            }
                        }.addOnFailureListener { exception ->
                            _registerResult.postValue(ResultState.Failure(errorMessage, "UPLOAD_IMAGE"))
                        }
                ...
                if (loginType == "Login as Admin") saveNewDataAdminToFirestore()
                else saveNewDataEmployeeToFirestore()
            }
        }
    }
...
//    }
    ...
    fun updateUserRolesAndProfile() {
        viewModelScope.launch {
            _registerResult.postValue(ResultState.Loading)
            try {
                val targetUsername = if (loginType == "Login as Admin") userAdminData.username else userEmployeeData.username
                val snapshot = withContext(Dispatchers.IO) {
                ...
                if (snapshot.isSuccessful) {
                    ...
                    if (document != null && document.exists()) {
                        ...
                        if (!isUsernameTaken) {
                            ...
                            val user = auth.currentUser
                            if (user != null) {
                                runBatchUpdate(user, targetUsername)
                            } else {
                                if (!NetworkMonitor.isOnline.value) {
                                    val errMessage = NetworkMonitor.errorMessage.value
                                    NetworkMonitor.showToast(errMessage, true)
...
//                                      _registerResult.postValue(ResultState.Failure("Koneksi internet tidak tersedia. Periksa koneksi Anda.", "UPDATE_ROLES"))
                                ...
                                val email = if (loginType == "Login as Admin") userAdminData.email else userEmployeeData.email
                                val password = if (loginType == "Login as Admin") userAdminData.password else userEmployeeData.password
                                auth.signInWithEmailAndPassword(email, password)
                                    .addOnCompleteListener { task ->
                                        if (task.isSuccessful) {
    ...
    // Fungsi pembantu untuk menjalankan Batch
    private suspend fun runBatchUpdate(user: FirebaseUser, targetUsername: String) {
        val success = withContext(Dispatchers.IO) {
            try {
                ...
                val phoneId = if (loginType == "Login as Admin") userAdminData.phone else userEmployeeData.phone

                val rolesRef = db.collection("users").document(phoneId)
                batch.set(rolesRef, userRolesCopy)

                val collectionName = if (loginType == "Login as Admin") "barbershops" else "employees"
