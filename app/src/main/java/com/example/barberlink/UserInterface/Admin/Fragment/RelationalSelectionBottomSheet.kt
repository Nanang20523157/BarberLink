 ...
 */
class RelationalSelectionBottomSheet : BottomSheetDialogFragment() {
    ...
    private fun setupObservers() {
        when (selectionType) {
            ...
            "BUNDLING" -> {
                ...
                addOutletViewModel.allServices.observe(viewLifecycleOwner) { list ->
                    packageAdapter?.allServices = list
                    // If we already have bundling, resubmit to update icons
                    packageAdapter?.currentList?.let { packageAdapter?.submitList(it) }
                }
    ...
    companion object {
        ...
        ): RelationalSelectionBottomSheet {
            return RelationalSelectionBottomSheet().apply {
                this.selectionType = type
                this.selectedIds = currentSelection.toMutableSet()
            }
        }
    }
}
